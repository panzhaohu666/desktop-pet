...
1. 在 `desktop_pet/skins/` 下新建文件夹（如 `我的角色`）
2. 放入一张透明 PNG 作为 `pet.png`（默认站姿）
3. 可选：创建序列帧动画（6~12 张 PNG 按 `01.png` `02.png` 命名）：
   ```
   click/jump/     → 单击时随机播放
   click/wiggle/   → 扭动
   click/spin/     → 旋转
   idle/           → 闲置打哈欠
   ```
4. 可选：放入 `phrases.json`，内容为 `["短语1","短语2",...]`
5. 右键 → 切换皮肤 → 选择你的角色

## ❓ 常见问题

**Q: 宠物窗口一闪就消失？**
A: 用 `python3 launch.py` 启动，不要直接 `&` 后台。Linux 需确保 xcb 库已装。

**Q: 怎么去掉音效？**
A: 右键 → 设置 → 行为 → 取消"启用音效"。

**Q: 双击为什么没反应？**
A: 快速连点两下（间隔 <300ms），不是慢速点两下。

**Q: 宠物卡住不动了？**
A: 右键 → 手动游走 试试。或者点系统托盘图标再点回来。

**Q: 想恢复出厂设置？**
A: 右键 → 设置 → 点"恢复默认设置"。

**Q: Windows 下载后报毒？**
A: PyInstaller 打包的误报，不是真病毒。解压 ZIP 后添加信任即可。
